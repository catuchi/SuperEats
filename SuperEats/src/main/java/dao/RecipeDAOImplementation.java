...
//    }
	...
	public List<Recipe> findRecipesByIngredient(String ingredient) {
		List<Recipe> recipes = new ArrayList<>();
		String sql = "SELECT r.* FROM Recipe r " + "JOIN RecipeIngredient ri ON r.recipeId = ri.recipeId "
				+ "JOIN Ingredient i ON ri.ingredientId = i.ingredientId " + "WHERE i.name LIKE ?";
	...
	public List<Recipe> findRecipesByCuisine(String cuisine) {
		List<Recipe> recipes = new ArrayList<>();
		String sql = "SELECT * FROM Recipe WHERE cuisineType LIKE ?";
